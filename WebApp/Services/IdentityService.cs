...
namespace WebApplicationAPI.Services {
  public class IdentityService : IIdentityService {
    ...
    public async Task<AuthenticationResult> RefreshTokenAsync(string token, string refreshToken) {
      ClaimsPrincipal? validatedToken = GetClaimsPrincipal(token);
      ...
      RefreshToken storedToken = await this.dbContext.RefreshTokens.SingleOrDefaultAsync(x => x.Token == refreshToken);
    ...
    private ClaimsPrincipal? GetClaimsPrincipal(string token) {
      ...
      try {
        ...
        if (!IsJwtWithSecurityAlgorithm(validatedToken)) return null;
        return principal;
      } catch {
        return null;
      }
